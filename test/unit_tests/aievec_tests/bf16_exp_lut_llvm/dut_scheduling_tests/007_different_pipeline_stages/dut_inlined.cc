...
extern "C" void dut(bfloat16 *restrict v1, bfloat16 *restrict v2) {
  ...
  aie::vector<uint16, 16> input_roll = aie::filter_even(input0).cast_to<uint16>();
  ...
  input_roll = aie::filter_even(input0).cast_to<uint16>();


  I_val_vec_roll = lookup_i.fetch(input);
  F_val_vec_roll = lookup_f.fetch(input);
